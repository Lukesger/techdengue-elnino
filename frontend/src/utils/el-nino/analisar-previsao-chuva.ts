...
export interface DiaPrevisaoLike {
  data?: string;
  periodo?: string;
  chuva_mm?: number;
  umidade_pct?: number;
...
export interface ClimaLike {
  fonte?: string;
  modo?: string;
  atual?: { condicao?: string };
  dias?: DiaPrevisaoLike[];
}
...
function round(v: number, casas = 1): number {
  const f = 10 ** casas;
...
export function analisarPrevisaoChuva(
  ...
  if (mensal) {
    ...
    const mesAnterior =
      fatia.length >= 2
        ? validarPrecipitacao(fatia.at(-2)!.chuva_mm)
        : 0;
  ...
  return {
    ...
    diasChuvaFortes: diario.filter((d) => d.mm >= 10).length,
    maxSeqChuva: maxSeq,
    maiorDia,
    umidMedia7,
...
export function detectarOndaCalor(dias: DiaPrevisaoLike[], clima?: ClimaLike) {
